/* Core module
   ...
   Note: most parsers require first type argument (input type) to be specified,
   otherwise the compiler will complain about 'cannot deduce function'.
...
auto
literal(I, S = string)(S str, bool consumeInput = true, bool caseSensitive = true) 
{
    import std.string;
...
unittest 
{
    ...
    auto p1 = literal!string("Hello");
    assert(p1.match(str));

    auto p2 = literal!string("hello", true, false);
    assert(p2.match(str));

    assert(!(p1 * p2).match(str));

    auto p3 = literal!string("Hello")
        * literal!string(" ")
...
auto
produce(I, O, S = string)(O returnValue)
{
    class Res: Parser!(I, O, S)
    {
        ParserState!(O, S) run(ParserState!(I, S) toParse)
...
/* Uses the same parser between 'min' and 'max' times. If either of 'min' and
   ...
   */
auto 
many(I, O, S = string)(Parser!(I, O, S) p, int min, int max)
{
    class Res: Parser!(I, O, S)
    {
        ParserState!(O, S) run(ParserState!(I, S) toParse)
...
unittest
{
    ...
    auto p1 = many(literal!string("1"), 3, 3);
    assert(p1.match(str1));
    assert(!p1.match(str2));

    auto p2 = many(literal!string("1"), 0, 4);
    assert(p2.match(str1));
    assert(p2.match(str2));

    auto p3 = many(literal!string("1"), 4, -1);
    assert(!p3.match(str1));
    assert(!p3.match(str2));

    auto p4 = many(literal!string("2"), 1, -1);
...
auto 
maybe(I, O, S = string)(Parser!(I, O, S) p)
{
...
/* Parses a single character that passes a test. Fails on empty strings and if
   ...
   */
auto
singleChar(I, T = char)(bool delegate(T) test)
{
    alias S = immutable(T)[];
    class Res: Parser!(I, S, S)
    {
...
unittest
{
    ...
    auto p1 = singleChar!string(ch => ch == 'f');
    assert(p1.match(str1));
    assert(!p1.match(str2));
    assert(!p1.match(str3));

    auto p2 = many(singleChar!string(ch => 'a' <= ch && ch <= 'e'), -1, -1);
...
unittest
{
    ...
    auto p = collectWhile!string(x => '0' <= x && x <= '9', false);
